function [output] = FcnFitAnisoTRMPSKinetics(time,uh,CFoptions)
...
[fitresult,gof,fitinfo] = fit(time(:), uh(:), corrFcn, 'StartPoint', startpoint,...
    'lower', lb,'upper',ub);
...
%     figure,clf
%     hold on
%     errorbar(time(ind1),uh,'rx')
%     plot(time(ind1),fitresult(time(ind1)))
%     hold off
%     xlabel('t_2 (ps)')
%     ylabel('c_2')
% end