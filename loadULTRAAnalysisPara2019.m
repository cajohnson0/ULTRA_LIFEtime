function d = loadULTRAAnalysisPara2019(data_file_search_string)
d = construct2dPP;
...
for ii = 1:length(files)
    ...
    d(ii).R  = temp(2:129,2:end).*-1000;
    d(ii).w3 = temp(2:129,1);
%     d(ii).comment = sprintf(files.name);
    
    %try to infer t2 time from the file name
    string = regexp(files(ii).name,'[\w_]([-+]?\d*.\d*)ps[\w_]','tokens');%'[\w_]([-+]?\d*.\d*)ps[\w_]','tokens');  
