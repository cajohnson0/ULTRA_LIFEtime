function [output] = GlobalSSDFit(data_file,c2,c2_std,tau_rot,CFOptions,Exp)
...
C2(x) = exp(-6.*Dm*x);
C3(x) = exp(-12.*Dm*x);
R_para(x) = (3/25)*((11.*C1(x) + 4*C3(x))./(1+0.8*C2(x)));
...
CFOptions.fitfcn = fittype( @(a1, tau1, x) a1.*exp(-x./tau1).*(double(R_perp(x))+double(R_para(x))),...
